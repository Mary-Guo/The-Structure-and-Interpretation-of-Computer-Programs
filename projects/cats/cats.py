...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    select_p = [item for item in paragraphs if select(item) == True]
    if k < len(select_p):
        return select_p[k]
    else:
        return ''
...
def about(topic):
    ...
    def helper(paragraphs):
        for item in topic:
            if item in split(lower(remove_punctuation(paragraphs))):
                return True
        return False
    return helper
...
def accuracy(typed, reference):
    ...
    if num_overlap == 0:
        return 0.0
    return num_overlap/len(typed_words) * 100
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in valid_words:
        if diff_function(user_word, i, limit) < diff:
            diff = diff_function(user_word, i, limit) 
            closest = i
    if diff_function(user_word, closest, limit) <= limit and closest != '':
        return closest
    return user_word
...
def fastest_words(game):
    ...
    big_list = [[] for i in range(len(players))]
    times = all_times(game)
    for j in range(len(words)):
        a = {}  #key would be the time, value would be the player index
        for i in players:
            if times[i][j] not in a:
                a[times[i][j]] = i
        big_list[a[min(a.keys())]].append(all_words(game)[j])
    return big_list
