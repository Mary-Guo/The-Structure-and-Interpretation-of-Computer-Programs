...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        i = 0
        #while len(place.bees) == 0 and place.entrance is not beehive:
        while i < self.min_range:
            i += 1
            place = place.entrance
        while i <= self.max_range and place is not beehive:
            if len(place.bees) != 0:
...
# END Problem 13
    ...
    def remove_from(self, place):
        if self.is_queen:
            ...
            #if place != None:
            ScubaThrower.remove_from(self, self.place)
    ...
    def action(self, gamestate):
        ...
        if not self.is_queen:
            ...
            while next_place.exit is not None:
                ...
                check_ants = next_place.ant
                if check_ants:
                    if check_ants not in self.buffed_ants:
                        check_ants.damage *= 2
                        self.buffed_ants.append(check_ants)
                    if isinstance(check_ants, ContainerAnt) and check_ants.contained_ant not in self.buffed_ants:
                        if check_ants.contained_ant != None:
                            check_ants.contained_ant.damage *= 2
                            self.buffed_ants.append(check_ants.contained_ant)
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        ant = self.place.ant
        if ant is None or ant.blocks_path == False:
            return False
        return True
        # END Problem 7
...
def apply_effect(effect, bee, duration):
    ...
    action_before = bee.action

    to_repeat = effect(action_before, bee)
    chain_0 = repeat(to_repeat, duration)
    chain_1 = repeat(action_before)

    to_be_looped = chain(chain_0, chain_1)

    def effected_action(gamestate):
        f = next(to_be_looped)
        action_effect = f(gamestate)
